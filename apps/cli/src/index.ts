  ...
  .action(async (path: string, options: { output: string }) => {
    ...
    try {
      ...
      if (options.output === 'json') {
        console.log(JSON.stringify({ callSites: allCallSites, errors }, null, 2));
      } else {
        console.log(chalk.bold('\nAPI Call Sites:'));
        for (const site of allCallSites) {
          console.log(`  ${chalk.cyan(site.filePath)}:${chalk.yellow(site.line)}`);
          console.log(`    ${chalk.green(site.method)} → ${chalk.blue(site.endpoint)}`);
        ...
        if (errors.length > 0) {
          ...
          for (const error of errors) {
            console.log(`  ${chalk.red(error.file)}:${chalk.yellow(error.line)} - ${error.message}`);
          }
        }
      }
    } catch (error) {
...
program
  ...
  .action(async (path: string, options: { command: string; timeout: string }) => {
    ...
    try {
      ...
      if (result.exitCode === 0) {
        ...
        spinner.fail(`Tests failed with exit code ${result.exitCode}`);
      }
...
program
  ...
  .action(async (path: string, options: { apiKey?: string }) => {
    ...
    try {
      ...
      if (!apiKey) {
        throw new Error('OpenAI API key is required. Use --api-key or set OPENAI_API_KEY');
      }
      ...
      console.log(chalk.dim('Note: This is a simplified demo. Full implementation requires snapshot comparison.'));
    } catch (error) {
