...
export async function handlePullRequest(opts: AppHandlePrOptions): Promise<GovernanceReport> {
  ...
  const ctx: any = { endpointId: opts.newSpec.endpoint, eventType: "pull_request", diff: { changes: summary.changes }, plan: { steps: [] } };
  ...
  if (summary.breakingChanges.length > 0 && violations.length === 0 && summary.riskScore.overall >= 70) {
    // risk-based block is handled in decideVerdict
  }

  const verdict = decideVerdict(summary, violations);
  const receipt = createVerdictReceipt(verdict, summary.changes, summary.riskScore);

  const changelog = {
    breaking: summary.breakingChanges,
    added: summary.nonBreakingChanges.filter((c) => c.includes("added")),
    changed: summary.nonBreakingChanges.filter((c) => c.includes("→") || c.includes("changed")),
    deprecated: summary.nonBreakingChanges.filter((c) => c.includes("optional")),
  };

  return { verdict, summary, receipt, policyViolations: violations, changelog };
}

export async function postPrComment(report: GovernanceReport, opts: { owner: string; repo: string; pr: number; token?: string }) {
